...
short wavInBuf[NUM_WAV_IN_BUFFERS][BufferSamples];

void (*waveInRdyCallback)(WAVEHDR* wh);

HWAVEIN hwi;
WAVEHDR wih[NUM_WAV_IN_BUFFERS]; 
...
void setupAudioIn(const char* deviceName, void (*waveInRdy)(WAVEHDR* wh)) {
  ...
  WAVEFORMATEX wfx = {WAVE_FORMAT_PCM, WAV_IN_CHANNELS,
                      ...
                      WAV_IN_CHANNELS * BITS_PER_SAMPLE / 8, BITS_PER_SAMPLE, sizeof(wfx)};  
  ...
  for (int b = 0; b < NUM_WAV_IN_BUFFERS; ++b) {
